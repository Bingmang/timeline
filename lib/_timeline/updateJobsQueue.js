 ...
 * @param {String} npm_dir 
 * @param {String} config_name 
 * @param {Object} job_configs 
 */
function updateJobsQueue(timeline, npm_dir, config_name, job_configs) {
  ...
  let package_dirs = $.fs.readdirSync(npm_dir)
  $._.remove(package_dirs, (package_dir) => {
    return package_dir[0] === '.'
  })
  // 对每个包 读取timeline.json和package.json获取参数及版本号 -> new_job_configs
  $._.map(package_dirs, (package_name) => {
    try {
      let package_json
      let timeline_json
      let package_info = $.fs.readFileSync(
        $.path.resolve(npm_dir, package_name, 'package.json'))
      let timeline_info = $.fs.readFileSync(
        $.path.resolve(npm_dir, package_name, config_name))
      package_json = JSON.parse(package_info)
      timeline_json = JSON.parse(timeline_info)
      // 严格检查所有参数
      let job_name = package_name + ' ' + package_json.version
      if (!$._.has(timeline_json, 'schedule', 'timeout', 'maintainer')) {
