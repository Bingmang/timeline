...
class Job {
   ...
   * @param {Agenda} agenda 
   * @param {object} script_config 
   * @param {string} job_name 
   */
  constructor(agenda, options, script_config) {
    this._agenda = agenda
    this._options = options
    this.package_config =
      $._.defaults(script_config, this._options.default_job_config)
    this._log = null
    this._log_err = null
    this._job = null
    this._process = null
  ...
  async remove() {
    try {
      await this._remove()
      console.log(
        `[+][TIMELINE] Remove job succesed: [${this.package_config.job_name}]`)
    } catch (err) {
      console.error(
        `[-][TIMELINE] Remove job failed: [${this.package_config.job_name}]`)
    }
  }
 ...
 */
  async execJob() {
    ...
    await $.retry(async (retry, number) => {
      console.log(
        `[+][JOB] Job runs: [${this.package_config.job_name}] ${number} times`)
      try {
        ...
        console.error('[-][JOB] Detected Error:', this.package_config.job_name)
        console.error(err)
        await this._abort()
        retry(err)
      } finally {
  ...
  async _abort() {
    ...
    console.log(
      '[!][JOB] Aborting the process:', this.package_config.job_name, pid)
    ...
    if (remain_pids.length) {
      ...
      console.log(
        '[!][JOB] Process success abort:', this.package_config.job_name, pid)
    }
  }
   ...
   */
  _defineJob() {
    this._agenda.define(this.package_config.job_name, this.package_config, 
      async (job, done) => {
        try {
          await this.execJob()
          done()
    ...
    this._job = this._agenda.every(
      this.package_config.schedule, this.package_config.job_name, {},
      { timezone: this._options.timezone })
    console.log('[+][TIMELINE] Timing job successed:', this.package_config.job_name)
  }
   ...
   */
  _outputConfigs() {
    console.log('[+][JOB] Startup prameters',
  ...
  _execScript() {
    return new $.Promise((resolve, reject) => {
      let [command, ...parameters] = this.package_config.script.split(' ')
      this._process = $.child_process.spawn(command, parameters, {
        cwd: $.path.resolve(
          this._options.npm_dir, this.package_config.package_dir),
        detached: true,
        // 日志重定向到文件
