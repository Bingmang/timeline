...
class Job {
  ...
  constructor(agenda, config) {
    ...
    // init log
    let job_log_dir =
      $.path.resolve($.conf.timeline.log_dir, this.package_name)
    ...
    this._job = agenda.every(config.schedule, this.job_name, {},
 ...
 */
  ...
  async _stop() {
    ...
    let remain_pids = await this._process_tree
      .checkAlive($.conf.timeline.process_check_delay)
    this._process_tree = null
    ...
    return new $.Promise((resolve, reject) => {
      ...
      this._process.on('exit', (code, signal) => {
        ...
        if (code === 1) {
          ...
          resolve()
          // 任务被强行退出
        } else if (signal === $.conf.timeline.process_exit_signal) {
          resolve(signal)
          // 未知退出状态
        } else {
